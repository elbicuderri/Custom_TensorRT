...
int MyInt8Calibrator::getBatchSize() const { return m_Batch; }

bool MyInt8Calibrator::getBatch(void* bindings[], const char* names[], int nbBindings)
{
	/*
...
const void* MyInt8Calibrator::readCalibrationCache(size_t& length)
{
	void* output{ nullptr };
...
void MyInt8Calibrator::writeCalibrationCache(const void* cache, size_t length)
{
	assert(!m_CalibTableFilePath.empty());
