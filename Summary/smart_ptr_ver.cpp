...
public:
	...
	{
		std::cout << "PoolingPlugin disappeared~" << std::endl;
	}
...
public:
	...
	virtual ~MyInt8Calibrator() 
	{ 
		cudaFree(m_DeviceInput);
		std::cout << "Calibrator disappeared~" << std::endl;
	}
...
int main()
{
	PrintMemoryUsage();
		...
		auto h_data = load_data_vector<float>("data/mnist_test_images_float32.bin");
		...
		std::unique_ptr<MyInt8Calibrator> calibrator2 = std::make_unique<MyInt8Calibrator>(calibration_number, 1,
			InputC, InputH, InputW, InputName, CalibrationFile, (float*)&calib_data[0]);
		PrintMemoryUsage();

		config->setInt8Calibrator(calibrator2.get());
		PrintMemoryUsage();
		...
		//std::unique_ptr<IPluginV2IOExt> my_pool = std::make_unique<PoolingPluginV2IO>(conv1->getOutput(0), BatchSize, 5, 24, 24,
		//	2, 2, 0, 0, 2, 2);

		std::unique_ptr<PoolingPluginV2IO> my_pool = std::make_unique<PoolingPluginV2IO>(conv1->getOutput(0), BatchSize, 5, 24, 24,
			2, 2, 0, 0, 2, 2);
		PrintMemoryUsage();
		...
		//std::shared_ptr<IPluginV2IOExt> shared_my_pool = my_pool;
