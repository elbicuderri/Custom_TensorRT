...
int main()
{
	...
	//MyInt8Calibrator calibrator(calibration_number, 1,
	//	InputC, InputH, InputW, InputName, CalibrationFile,
	//	std::vector<float>(h_data.begin(), h_data.begin() + (calibration_number * InputH * InputW)));
	...
	ITensor* const s = scale_1->getOutput(0);

	IPluginV2Layer* ConvPluginV2Layer = network->addPluginV2(&s, 1, *my_conv2);
	std::cout << "IpluginLayer made." << std::endl;
	ConvPluginV2Layer->getOutput(0)->setName("conv_plugin");

	//ConvPluginV2Layer->setOutputType(0, DataType::kINT8);
	//ConvPluginV2Layer->setPrecision(DataType::kINT8);
	...
	for (int epoch = 0; epoch < epochs; ++epoch)
	{
		...
		h_batchdata = std::vector<float>(h_data.begin() + ((epoch + calibration_number) * BatchSize * InputH * InputW),
			h_data.begin() + ((epoch + calibration_number) * BatchSize * InputH * InputW + BatchSize * InputH * InputW));
