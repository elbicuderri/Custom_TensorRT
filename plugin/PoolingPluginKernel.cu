...
__global__ void maxpool_plugin_kernel(float *output, float *input,
	...
	int kernel_height, int kernel_width, int pad_height, int pad_width, int stride_height, int stride_width, cudaStream_t stream)
{
	...
	maxpool_plugin_kernel << < NUMBER_OF_BLOCKS, THREADS_PER_BLOCK, 0, stream >> > (output, input, N, C, H, W, kH, kW, pH, pW, sH, sW, TOTAL_SIZE);
	...
	int kernel_height, int kernel_width, int pad_height, int pad_width, int stride_height, int stride_width, cudaStream_t stream)
{
	...
	maxpool_plugin_int8_kernel << < NUMBER_OF_BLOCKS, THREADS_PER_BLOCK, 0, stream >> >
		(output, input, N, C, H, W, kH, kW, pH, pW, sH, sW, TOTAL_SIZE);
